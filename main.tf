...
resource "random_id" "uniq_nums" {
  byte_length = 8
}

resource "aws_s3_bucket" "demo_bucket" {
  bucket = format("%s-%s", var.bucket_name, random_id.uniq_nums.hex)
  tags = {
    Name = "demo"
    Env  = "Test"
...
# 2 upload the objects to s3 bucket
resource "aws_s3_object" "bk_test" {
...
# 3 Enabled Server Side Encryption using KMS key
...
resource "aws_kms_alias" "s3_bucket_kms_key_alias" {
  name          = "alias/test_bucket_kms_key_alias"
...
# 4 Read only policy for s3 bucket
# Disable the Public access policy
...
resource "aws_s3_bucket" "log_bucket" {
  bucket = format("%s-%s", "my-demo-test-logging", random_id.uniq_nums.hex)
  tags = {
    Name        = "My log bucket"
    Environment = "test"
  }
